...
var PagerView = function(id){
	...
	this.render = function(){
		...
		if(self._pageCount > 1){
			if(self.index != 1){
				str += '<a href="javascript://1"><span class="btn">首页</span></a>';
			}else{
				str += '<span class="btn">首页</span>';
			}
		}
		for(var i=self._start; i<=self._end; i++){
			if(i == this.index){
				str += '<span class="on btn">' + i + "</span>";
			}else{
				str += '<a href="javascript://' + i + '"><span class="btn">' + i + '</span></a>';
			}
		}
		if(self._pageCount > 1){
			if(self.index != self._pageCount){
				str += '<a href="javascript://' + self._pageCount + '"><span class="btn">尾页</span></a>';
			}else{
				str += '<span class="btn">尾页</span>';
			}
		}
		str += '</div><!-- /.pagerView -->\n';
